...
PROPOSAL_POLICY = (
    ...
    "recent_events lists the calendar entries of the last 14 days, possibly trimmed: "
    "compare it against recent_activities and say which prescribed session each "
    "executed activity does or does not match.\n"
    ...
    "Hike or Walk have no pace model, so any step makes Intervals recompute duration and "
    "load: use a plain description with no step lines, and rewrite an existing distance "
    "step when revising one; set moving_time, distance and load explicitly.\n"
