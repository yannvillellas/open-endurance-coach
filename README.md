...
- **When he proposes calendar changes**, he asks: "Apply this to Intervals.icu: …". Reply with exactly `yes` and the changes are validated, approved, and written in one step (if the write fails, say `retry` to apply the recorded decision again). `no` declines, and **anything else is a change request** — he re-analyzes with your words and proposes again. Nothing is ever written without a literal yes.
...
| `HISTORY_DAYS`                     | `180`            | Stored history kept (drafts, decisions, feedback, seen activities); `0` keeps forever                                         |
