...
- **Implication:** our writer sets a native Intervals `description` (time/HR steps) plus the planned targets; structured `.zwo` upload is a reference capability we do not use (description-based only). A read-back after every write reports the plan targets Intervals changed, any computed duration that differs from the plan, and any distance step it derived; a create that returns no id is flagged as unverified.
