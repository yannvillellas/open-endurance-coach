...
- **Targets vs computed (probe-verified 2026-09-19):** `time_target`, `distance_target` and `load_target` persist as sent on a `WORKOUT`, while `distance` is derived from the parsed steps and our value is echoed back as 0. On a `RACE_*` event `distance` is first-class and persists. The writer therefore maps a mutation's `distance` to `distance_target` for workouts and to `distance` for races, and mirrors `moving_time`/`icu_training_load` into `time_target`/`load_target`.
