...
- **Intervals (`icu_intervals[]`):** ids are ints; `start_time`/`end_time` are seconds within the activity, not epochs; `type`/`zone`/`intensity` are Intervals-generated classifications. They are not a reliable split source: the 2025-09-28 race carries a single whole-activity interval (`3761s@183bpm`, verified 2026-09-19).
- **Streams and splits (verified live 2026-09-19):** `GET /activity/{id}/streams?types=time,distance,altitude,heartrate` returns the per-sample series as `[{type, data}]` (a `.csv` body is also available). We compute per-kilometre splits from those streams (`extractors/splits.py`: distance, time, pace, average/max HR, elevation gain and loss, net grade) and add them to a deep-query context as `activity_splits`, omitted when empty. No interval has to exist on the activity and nothing is written to Intervals. Rides report average speed and power per split and request the `watts` stream; runs and hikes report pace and do not request it. Rows are capped at forty by merging consecutive kilometres (per-km up to 40 km; a 150 km ride becomes ~4 km rows), so a long ride stays bounded and labelled (`km 1-4`).
