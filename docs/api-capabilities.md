...
- Payload fields for a workout event: `category: WORKOUT`, `start_date_local`, `name`, `description`, `type` (e.g. `Ride`), `moving_time` (seconds), `distance_target` (metres, the planned distance), `icu_training_load`, optionally `color`, `folders_id`, etc.
- **Planned distance is `distance_target`, not `distance` (probe-verified 2026-09-19):** on a `WORKOUT`, `distance` is derived from the parsed steps and our value is discarded (`distance` echoed back as 0); `distance_target` persists (probe: sent 8690, read back 8690). On a `RACE_*` event `distance` is a first-class field and persists as sent. The writer maps the mutation's `distance` to `distance_target` for workouts and to `distance` for races.
- **Server recompute (verified live 2026-09-19):** any parsed description step makes Intervals own the event's duration, distance and load — it parses the step into `workout_doc.steps` and rewrites `moving_time`, `distance` and `icu_training_load`, overwriting what we sent. Units matter: `1h49` is read as 1 h + 49 **seconds** (stored 3649 s); write `1h49m`. A **plain description with no step lines** leaves our `moving_time` and `icu_training_load` intact (probe: 9360 s and load 62 kept). `Hike` and `Walk` are in no sport-settings group, so a distance step is priced with a running pace (measured at 10.0 km/h). The contract therefore uses a plain description for Hike/Walk, and the writer's read-back reports any field Intervals still changed.
