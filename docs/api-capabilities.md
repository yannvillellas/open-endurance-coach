...
- Payload fields for a workout event: `category: WORKOUT`, `start_date_local`, `name`, `description`, `type` (e.g. `Ride`), `moving_time` (seconds), `distance` (metres), `icu_training_load`, optionally `color`, `folders_id`, etc.
- **Server recompute (verified live 2026-09-19):** a description step can carry a distance target (`- 12.4km Z1 HR`). Intervals then parses it into `workout_doc.steps` and rewrites the event's `duration`/`moving_time`, `distance` and `icu_training_load` from its pace model, overwriting the values we sent. `Hike` and `Walk` are in no sport-settings group, so they are priced with a running pace (measured at 10.0 km/h). Put distance in the `distance` field and keep the description time-based or plain for those types; the writer's read-back reports any field Intervals changed.
